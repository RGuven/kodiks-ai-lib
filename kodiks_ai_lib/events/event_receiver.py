...
class EventReceiver(object):
    def __init__(self, username, password, host, port, queue_name, service, service_name, logger):
        ...
        self.service_instance = self.service_worker()  # Loaded model via init

        # If service_instance has build_model function run it
        if hasattr(self.service_instance, "build_model"):
            self.service_instance.build_model()
    ...
    def on_request(self, ch, method, props, body):
        ...
        try:
            response, task_type = self.service_instance.call(body)
            ...
            self.logger_helper(
                props.correlation_id, self.queue_name, self.service_name, self.logger, "end", "Receiver exception"
            )
